...
-- ---------- 015: Charge per audit attempt ----------
-- `charge_credits` keyed the ledger on the contract, which made a retry after a
-- failed audit free — and every later audit of a paid contract free with it,
-- while the route went on to run OCR, retrieval and the model again. Retrying
-- and re-running differ by whether the previous attempt delivered anything, not
-- by whether the contract has ever been paid for: a failure refunds, a finished
-- audit does not. So the ledger keys on the attempt, and the free retry becomes
-- what it always meant — the refund.
